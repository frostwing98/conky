...
_TOP_COLORS = {"#00ff00","#33ff00","#66ff00","#99ff00","#ccff00","#ffff00","#ffcc00","#ff9900","#ff6600","#ff3300","ff0000"}
_TOP_COLORS_SIZE = 11
_BAT_COLORS = {"ff0000","#ffff00","#00ff00"}
RED_START = 0x44
RED_END = 0xff
GREEN_START = 0xFF
GREEN_END = 0x00
    ...
    if tonumber(conky_parse("$updates")) % 4 == 0 then
        ...
        for line in file:lines() do
            ...
            if count <= tonumber(n) then
                media = media
                        .. "${goto 10}" .. "${color ".. COLORS[(count % table.getn(COLORS))+1] .."}" .. short_name .. "${goto  90}${fs_bar 7,150 " .. trim(line)
                        .. "}${goto 250}${fs_used " .. trim(line) .. "}/${fs_size " .. trim(line) .. "}" .. "$color"
                        .. "\n"
            else
                break
            end
        ...
        for c = 1, tonumber(ncpu)  do
            if c % 2 ~= 0 then
                conky_cpus = conky_cpus
                             .. "${goto 10}${color ".. (ENABLE_COLORS and get_color_freq_cpu(c-1) or "#FFFFFF").."}" .. c ..": ${goto 30}${cpu cpu".. c
                             .."}%${goto 70}${cpubar cpu".. c
                             .." 7,50}${goto 110}  ${freq_g ".. c
                             .."}GHz${goto 190}${color ".. (ENABLE_COLORS and get_color_freq_cpu(c) or "#FFFFFF") .."}|".. c+1 
                             ..":${goto 220}${cpu cpu".. c+1
                             .."}%${goto 260}${cpubar cpu".. c+1 .." 7,50}${goto 320}${freq_g ".. c+1 .."}GHz"
                             .. "${color}\n"
            end
        end
        cpus = conky_cpus
            ...
            for i, iface in pairs(active_ifaces) do
                if i <= tonumber(n) then
                    draw_other_ifaces = draw_other_ifaces
                                        .. "${goto 10}".. "${color ".. COLORS[(i % table.getn(COLORS)) + 1] .."}" .. i ..". "
                                        .. iface .." "..  "${font FontAwesome} ${font}${voffset 0} ${addrs " .. iface ..  "}${color}" .. "\n"
                                        .. "${goto 10}${upspeedgraph " .. iface ..  " 20,180 00ffff 00ff00}${goto 200}${downspeedgraph "
                                        .. iface ..  " 20,180 FFFF00 DD3A21}" .. "\n"
                                        .. "${font FontAwesome}${goto 80}${font} ${upspeed "
                                        .. iface ..  "}${font FontAwesome}${goto 270}${font} ${downspeed " .. iface ..  "}" .. "\n"
...
function conky_cputemp()
    ...
    return "${color #ffff00}${font FontAwesome}${voffset 0}   ${voffset 0}${color #ffffff}${font}" .. ctemp
end
...
function conky_topprocess(n)
    ...
    if topprocesses == '' then
        topprocesses = topprocesses .. "${color #00FF00}${goto 10}Name ${goto 240}Pid${goto 290}Cpu%${goto 350}Mem%${color}\n"
        for p = 1, tonumber(n) do
            if ENABLE_COLORS then
                col = "${color " .. _TOP_COLORS[(_TOP_COLORS_SIZE + 1) - p] .. "}"
            end
            topprocesses = topprocesses
                        .. col .. "${goto 10}${top name ".. p .. "} ${goto 220}${top pid ".. p .. "}${goto 270}${top cpu ".. p
                        .. "}${goto 330}${top mem ".. p .. "}${color}\n"
        end   
    end
    return topprocesses
end
